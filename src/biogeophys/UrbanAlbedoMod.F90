...
contains
  ...
  subroutine UrbanAlbedo (bounds, num_urbanl, filter_urbanl, &
    ...
    real(r8) :: sdif_roof     (bounds%begl:bounds%endl, numrad) ! diffuse solar radiation incident on roof per unit incident flux
    real(r8) :: sdif_ar_tree      (bounds%begl:bounds%endl, numrad) ! diffuse solar radiation incident on above-roof tree canopy per unit incident flux
    real(r8) :: sdif_br_tree       (bounds%begl:bounds%endl, numrad) ! diffuse solar radiation incident on below-roof tree canopy per unit incident flux
    ...
    real(r8) :: alb_br_tree_dir_s  (bounds%begl:bounds%endl, numrad) ! direct below-roof tree albedo with snow effects
    ...
    real(r8) :: alb_br_tree_dif_eff     (bounds%begl:bounds%endl, numrad) ! effective diffuse below-roof tree albedo (albedo + transmittance)
    real(r8) :: alb_ar_tree_dif_eff     (bounds%begl:bounds%endl, numrad) ! effective diffuse above-roof tree albedo (albedo + transmittance)
   ...
   ! Timekeeping variables can be deleted in the final version.
    ...
    integer  :: num_nourbtreesol                                                              ! number of unvegetated patches where coszen>0
    integer  :: filter_urbtreesol   (bounds%endp-bounds%begp+1)                               ! patch filter where vegetated and coszen>0
    integer  :: filter_nourbtreesol (bounds%endp-bounds%begp+1)                               ! patch filter where unvegetated and coszen>0                                                   
    !-----------------------------------------------------------------------
    ...
    associate(                                                        &
         ...
         alb_improad_dir    => urbanparams_inst%alb_improad_dir     , & ! Input: [real(r8) (:,:) ]  direct impervious road albedo                   
         alb_improad_dif    => urbanparams_inst%alb_improad_dif     , & ! Input: [real(r8) (:,:) ]  diffuse imprevious road albedo                  
         ...
         alb_ar_tree_dir       => urbanparams_inst%alb_tree_urb_dir        , & ! Input: [real(r8) (:,:) ]  direct above-roof tree albedo  
         alb_ar_tree_dif       => urbanparams_inst%alb_tree_urb_dif        , & ! Input: [real(r8) (:,:) ]  diffuse above-roof tree albedo  
         ...
         sabs_br_tree_dir      =>    solarabs_inst%sabs_br_tree_dir_lun      , & ! Output: [real(r8) (:,:) ]  direct  solar absorbed  by below-roof tree per unit tree area per unit incident flux
         sabs_ar_tree_dir      =>    solarabs_inst%sabs_ar_tree_dir_lun      , & ! Output: [real(r8) (:,:) ]  direct  solar absorbed  by above-roof tree per unit tree area per unit incident flux
         sabs_roof_dif      =>    solarabs_inst%sabs_roof_dif_lun      , & ! Output: [real(r8) (:,:) ]  diffuse solar absorbed  by roof per unit roof area per unit incident flux
         ...
         sabs_br_tree_dif      =>    solarabs_inst%sabs_br_tree_dif_lun      , & ! Output: [real(r8) (:,:) ]  diffuse solar absorbed  by below-roof tree per unit tree area per unit incident flux
         sabs_ar_tree_dif      =>    solarabs_inst%sabs_ar_tree_dif_lun      , & ! Output: [real(r8) (:,:) ]  diffuse solar absorbed  by above-roof tree per unit tree area per unit incident flux
         sabs_tree_dir   => solarabs_inst%sabs_tree_dir_lun   , & ! Output: [real(r8) (:,:) ]  direct  solar absorbed  by tree per unit tree area per unit incident flux
         sabs_tree_dif   => solarabs_inst%sabs_tree_dif_lun   , & ! Output: [real(r8) (:,:) ]  diffuse solar absorbed  by tree per unit tree area per unit incident flux

         sref_roof_dir   => solarabs_inst%sref_roof_dir_lun   , & ! Output: [real(r8) (:,:) ]  direct  solar reflected by roof per unit roof area per unit incident flux
         ...
         sref_br_tree_dir     => solarabs_inst%sref_br_tree_dir_lun     , & ! Output: [real(r8) (:,:) ]  direct  solar reflected by below-roof tree per unit tree area per unit incident flux
         sref_ar_tree_dir     => solarabs_inst%sref_ar_tree_dir_lun     , & ! Output: [real(r8) (:,:) ]  direct  solar reflected by above-roof tree per unit tree area per unit incident flux
         sref_roof_dif   => solarabs_inst%sref_roof_dif_lun   , & ! Output: [real(r8) (:,:) ]  diffuse solar reflected by roof per unit roof area per unit incident flux
         ...
         sref_br_tree_dif     => solarabs_inst%sref_br_tree_dif_lun     , & ! Output: [real(r8) (:,:) ]  diffuse solar reflected by below-roof tree per unit tree area per unit incident flux
         sref_ar_tree_dif     => solarabs_inst%sref_ar_tree_dif_lun     , & ! Output: [real(r8) (:,:) ]  diffuse solar reflected by above-roof tree per unit tree area per unit incident flux
         sref_tree_dir   => solarabs_inst%sref_tree_dir_lun   , & ! Output: [real(r8) (:,:) ]  direct  solar reflected  by pervious road per unit tree area per unit incident flux
         sref_tree_dif   => solarabs_inst%sref_tree_dif_lun   , & ! Output: [real(r8) (:,:) ]  diffuse solar reflected  by pervious road per unit tree area per unit incident flux
...
! end add new snicar
         ! These two variables are used in twostream module
         albd_perroad_s         => solarabs_inst%albd_perroad_s_lun          , & ! Output:  [real(r8) (:,:) ]  ground direct solar albedo on pervious road with snow effects
         albi_perroad_s         => solarabs_inst%albi_perroad_s_lun          , & ! Output:  [real(r8) (:,:) ]  ground diffuse solar albedo on pervious road with snow effects
         begc               => bounds%begc                          , &
         endc               => bounds%endc                          , &
         
         begl               => bounds%begl                          , &
         krs1d              =>    urbanparams_inst%ksv1d_out , & ! Input:  [real(r8) (:,:) ]  Monte carlo view factor from roof to sky [landunit, nzcanm]               
         kws1d              =>    urbanparams_inst%kws1d_out  , & ! Input:  [real(r8) (:,:) ]    Monte carlo view factor from wall to sky[landunit, nzcanm]          
         kvs1d              =>    urbanparams_inst%kvs1d_out  , & ! Input:  [real(r8) (:,:) ]    Monte carlo view factor from vegetation to sky[landunit, nzcanm] 

         svfw_k              =>    urbanparams_inst%svfw_k_out , & ! Input:  [real(r8) (:,:) ]  Unweighted monte carlo view factor from wall to sky [landunit, nzcanm]               
         svfv_k              =>    urbanparams_inst%svfv_k_out  , & ! Input:  [real(r8) (:,:) ]    Unweighted monte carlo view factor from vegetation to sky[landunit, nzcanm]          
         svfr_k              =>    urbanparams_inst%svfr_k_out  , & ! Input:  [real(r8) (:,:) ]    Unweighted monte carlo view factor from roof to sky[landunit, nzcanm] 
         svft_k              =>    urbanparams_inst%svft_k_out  , & ! Input:  [real(r8) (:) ]    Unweighted monte carlo view factor from ground to sky[landunit, nzcanm] 
         endl               => bounds%endl                            &
         )
      ...
      ! create coszen_patch which is required to create solar-urbantree filter and twostream module
      ...
      ! Assign urban tree LAI, albedo, and transmittance to patches in filter_urbantreep, and compute the effective albedo of below-roof and above-roof tree canopy (albedo + transmittance)
      do fp = 1,num_urbantreep
         p = filter_urbantreep(fp)
         l = patch%landunit(p)
         ...
         ! the effective "albedo" of below-roof and above-roof tree is the sum of the tree albedo and transmittance (used for multiple reflection)
         alb_br_tree_dir_eff(l,:) =     alb_br_tree_dir(l,:) + tran_br_tree_dir(l,:)
         alb_ar_tree_dir_eff(l,:) =     alb_ar_tree_dir(l,:) + tran_ar_tree_dir(l,:)
         alb_br_tree_dif_eff(l,:) =     alb_br_tree_dif(l,:) + tran_br_tree_dif(l,:)
         alb_ar_tree_dif_eff(l,:) =     alb_ar_tree_dif(l,:) + tran_ar_tree_dif(l,:) 
      end do
      ...
      do ib = 1, numrad
         ...
         do fp = 1,num_urbanp  
            p = filter_urbanp(fp)
            l = patch%landunit(p)
            ! Setting albedos to view factors ensures that urban
            ! albedo will scale up to 1.0
            ! Discuss: why this is necessary?
            c = patch%column(p)
            ! revise: check the dimension index
            if (col%itype(c) == icol_sunwall) then
               albd(p,ib) = svfw_k(l,1)!vf_sw(l)
               albi(p,ib) = svfw_k(l,1)!vf_sw(l)
            else if (col%itype(c) == icol_shadewall) then
               albd(p,ib) = svfw_k(l,1)!vf_sw(l)
               albi(p,ib) = svfw_k(l,1)!vf_sw(l)
            else if (col%itype(c) == icol_road_perv .or. col%itype(c) == icol_road_imperv) then
               albd(p,ib) = svft_k(l)!vf_sr(l)
               albi(p,ib) = svft_k(l)!vf_sr(l)
            else if (col%itype(c) == icol_roof) then
               albd(p,ib) = svfr_k(l,2)
               albi(p,ib) = svfr_k(l,2)
            else if (col%itype(c) == icol_road_tree) then
               albd(p,ib) = svfv_k(l,1)
               albi(p,ib) = svfv_k(l,1)            
            endif
            fabd(p,ib)     = 0._r8
            fabd_sun(p,ib) = 0._r8
            fabd_sha(p,ib) = 0._r8
      ...
      do ib = 1,numrad
         do fl = 1,num_urbanl
            ...
            sref_roof_dir(l,ib)      = svfr_k(l,2)
            sref_roof_dif(l,ib)      = svfr_k(l,2)
            
            ! Setting sref to wall and road view factors ensures that urban
            ! albedo will scale up to 1.0
            ! revise: check the dimension index
            sref_sunwall_dir(l,ib)   = svfw_k(l,1)
            sref_sunwall_dif(l,ib)   = svfw_k(l,1)
            sref_shadewall_dir(l,ib) = svfw_k(l,1)
            sref_shadewall_dif(l,ib) = svfw_k(l,1)
            sref_improad_dir(l,ib)   = svft_k(l)
            sref_improad_dif(l,ib)   = svft_k(l)
            sref_perroad_dir(l,ib)   = svft_k(l)
            sref_perroad_dif(l,ib)   = svft_k(l)
            sref_br_tree_dir(l,ib)   = svfv_k(l,1)
            sref_br_tree_dif(l,ib)   = svfv_k(l,1)
            sref_ar_tree_dir(l,ib)   = svfv_k(l,2)
            sref_ar_tree_dif(l,ib)   = svfv_k(l,2)
            sref_tree_dir(l,ib)   = svfv_k(l,1)
            sref_tree_dif(l,ib)   = svfv_k(l,1)
         end do
      end do
      ...
      if (num_solar > 0)then
         ...
         ! Note that h2osno_total(c) hasn't been set for urban tree yet
         if (num_urbanl > 0) then
            ic = 0
         ...
         ! revise
         ! Note that frac_sno(c) hasn't been set for urban tree yet
         do ib = 1,numrad
            do fc = 1,num_urbanc
               c = filter_urbanc(fc)
               l = col%landunit(c)
               if (ctype(c) == icol_roof) then    
                  ...
                  albd_perroad_s(l,ib) = alb_perroad_dir_s(l,ib) 
                  albi_perroad_s(l,ib) = alb_perroad_dif_s(l,ib) 
...
! end add new snicar
         ...
         ! The required input temperature_inst%t_veg_patch is replaced by t_grnd
         !------------------------------------------------------------------------
         ! check whether this call would interferen any canyon-aware tree radiation; who overwrites whom?
         ! correct: use filter_urbtreesol, num_urbtreesol instead of filter_urbantreep, num_urbantreep
         call TwoStream (bounds, filter_urbtreesol, num_urbtreesol, &
         ...
         do ib = 1,numrad
            do fp = 1,num_nourbtreesol
               p = filter_nourbtreesol(fp)
               c = patch%column(p)
               fabd(p,ib)     = 0._r8
               fabd_sun(p,ib) = 0._r8
               fabd_sha(p,ib) = 0._r8
...
!-----------------------------------------------------------------------  
  function gaussian_quadrature(f, a, b, x, w,l) result(integral)
    ...
    quad_sum = 0.0_r8
    do i = 1, n
        quad_sum = quad_sum + w(i) * f(0.5_r8 * (b - a) * x(i) + 0.5_r8 * (b + a), l)
    end do
    
    integral = 0.5_r8 * (b - a) * quad_sum
...
!---------------------------------------------------------------------------------
  subroutine incident_direct (bounds,num_urbanl, filter_urbanl, canyon_hwr, wtlunit_roof,&
    ...
    real(r8), intent(in) :: h1( bounds%begl: )      ! Height of tree canopy crown below roof (m)
    real(r8), intent(in) :: h2( bounds%begl: )      ! Depth of tree canopy crown (m)
    ...
    real(r8), intent(out) :: sdir_road( bounds%begl: , 1: )      ! direct beam solar radiation (per unit road area)incident on road per unit incident flux with tree attenuation [landunit, numrad]
    ...
    real(r8) ::  sdir_tree_road(bounds%begl:bounds%endl,numrad)! direct beam solar radiation (per unit wall area) attenuated by above roof tree before reaching to road
    real(r8) ::  sdir_tree_sunwall(bounds%begl:bounds%endl,numrad)! direct beam solar radiation (per unit wall area) attenuated by above roof tree before reaching to sun-lit wall
   ...
   do fl = 1,num_urbanl
      ...
      if (coszen(l) > 0._r8) then
         ...
         if (lai(l) > 1e-6_r8) then
            ...
            if (h2(l)+h1(l) > ht_roof(l)) then  ! tree above roof scenario
   ...
   do ib = 1,numrad
      do fl = 1,num_urbanl
         ...
         if (coszen(l) > 0._r8) then   
            if (lai(l) > 1e-6_r8) then
               ! The latdeg and londeg are only used for the printouts for diagnosis purpose; can be removed later
               latdeg=grc%latdeg(g)
               londeg=grc%londeg(g)
               ! Initialization
               sdir_shadewall(l,ib) = 0._r8  
               ...
               if (ca_order(l)==0) then !tree below roof
                  ...
                  if (zen(l)>zen_ar_wr5(l)) then
                     ! correct: gaussian_quadrature(integrand_ar_BW_12, theta_ar_wr5(l), theta_max, Gauss_nodes, Gauss_weights,l)) / theta_max
                     ! to gaussian_quadrature(integrand_tree_partition_5pi, theta_ar_wr5(l), theta_max, Gauss_nodes, Gauss_weights,l)) / theta_max
                     sdir_after_ar(l,ib) = (tree_partition_05(theta_ar_wr5(l),l)&
                                 + gaussian_quadrature(integrand_tree_partition_5pi, theta_ar_wr5(l), theta_max, Gauss_nodes, Gauss_weights,l)) / theta_max
                  else      
               ...
               ! Here A_v1 is two-sided leaf area, which is consistent with leaf area used for view factor normalization
               ...
               if (sdir_tree_roof(l,ib) < 0._r8 .or. sdir_tree_sunwall(l,ib) < 0._r8 .or. sdir_tree_road(l,ib) < 0._r8 &
               .or. sdir_ar_tree(l,ib) < 0._r8  .or. sdir_br_tree(l,ib) < 0._r8 ) then
                  write (iulog,*) 'ERROR: negative tree-attenuated direct solar flux'
                  write (iulog,*) '  sdir_tree_roof(l,ib)    = ', sdir_tree_roof(l,ib)
                  write (iulog,*) '  sdir_tree_sunwall(l,ib) = ', sdir_tree_sunwall(l,ib)
                  write (iulog,*) '  sdir_tree_road(l,ib)    = ', sdir_tree_road(l,ib)
                  write (iulog,*) '  sdir_ar_tree(l,ib) = ', sdir_ar_tree(l,ib)
                  write (iulog,*) '  sdir_br_tree(l,ib) = ', sdir_br_tree(l,ib)
               ...
               if (abs(err1(l)) > 0.001_r8) then
                  write (6,'(A,2F10.3)') 'swall_projected, sdir_road(l,ib)',swall_projected, sdir_road(l,ib)
                  write (iulog,*) 'urban direct beam solar radiation balance error',err1(l)
                  write (iulog,*) 'clm model is stopping'
                  call endrun(subgrid_index=l, subgrid_level=subgrid_level_landunit, msg=errmsg(sourcefile, __LINE__))
               endif  
         ...
         ! write(iulog,*) 'DIRECT INCIDENT DEBUG l=', l, ' ib=', ib
         ! write(iulog,*) 'ca_order=', ca_order(l)
         ! write(iulog,*) 'sdir=', sdir(l,ib)
         ...
         ! write(iulog,*) 'sdir_after_ar=', sdir_after_ar(l,ib)
         ! write(iulog,*) 'sdir_ar_tree=', sdir_ar_tree(l,ib)
         ! write(iulog,*) 'sdir_br_tree=', sdir_br_tree(l,ib)
         ! write(iulog,*) 'A_v1=', A_v1(l), ' A_v2=', A_v2(l), ' A_g=', wcan(l), ' A_w=', ht_roof(l)
         ! write(iulog,*) 'ground-area direct residual below roof=', &
         !    sdir_after_ar(l,ib) - sdir_road(l,ib) - &
         !    (sdir_sunwall(l,ib) + sdir_shadewall(l,ib))*canyon_hwr(l)
         ! write(iulog,*) 'ground-area direct residual no above-roof=', &
         !    sdir(l,ib) - sdir_road(l,ib) - &
         !    (sdir_sunwall(l,ib) + sdir_shadewall(l,ib))*canyon_hwr(l)
  ...
  subroutine incident_diffuse (bounds, &
    ...
    real(r8)              , intent(in)  :: wtlunit_roof     ( bounds%begl: )      ! urban landunit weight of road tree column to total road (-)
    ...
    associate(                            & 
      ...
      do ib = 1, numrad
         ...
         do fl = 1,num_urbanl
            ...
            !     debug_write=.false. 
            ! end if
            ! A_w:building height; [m]
            ! A_r:building width; [m]
            ! A_g:road width; [m]
            ! A_v1/v2: normalized tree leaf area index below/above roof; [m]
            A_w=ht_roof(l)
            A_r=ht_roof(l)/(canyon_hwr(l)*(1._r8-wtlunit_roof(l))/wtlunit_roof(l))
            A_g=ht_roof(l)/canyon_hwr(l)
            A_s=A_r+A_g
   
            sdif_road(l,ib)      = sdif(l,ib) * ksg1d(l) !ksg1d=vfst*A_s/A_g slightly different from CLM; resulting flux is in respect to road area
  ...
  subroutine net_solar (bounds                                                                 , &
    ...
    real(r8), intent(in)    :: wtlunit_roof         ( bounds%begl: )        ! weight of roof with respect to landunit  [landunit]
    real(r8), intent(in)    :: ht_roof               ( bounds%begl: )             ! height of urban roof (m) [landunit]
    real(r8), intent(in)    :: A_v1               ( bounds%begl: )             ! Leaf area for urban tree canopy below roof (m) [landunit]
    ...
    real(r8), intent(in)    :: alb_br_tree_dir    ( bounds%begl: , 1: ) ! direct below-roof tree foliagealbedo [landunit, numrad]
    real(r8), intent(in)    :: alb_br_tree_dif    ( bounds%begl: , 1: ) ! diffuse below-roof tree foliagealbedo [landunit, numrad]
    real(r8), intent(in)    :: alb_ar_tree_dir    ( bounds%begl: , 1: ) ! direct above-roof tree foliagealbedo [landunit, numrad]
    real(r8), intent(in)    :: alb_ar_tree_dif    ( bounds%begl: , 1: ) ! diffuse above-roof tree foliagealbedo [landunit, numrad]
    real(r8), intent(in)    :: sdir_road          ( bounds%begl: , 1: ) ! direct beam solar radiation (per unit road area) incident on road per unit incident flux [landunit, numrad]
    ...
    real(r8), intent(in)    :: sdif_road          ( bounds%begl: , 1: ) ! diffuse solar radiation (per unit road area) incident on road per unit incident flux [landunit, numrad]
    ...
    real(r8), intent(in)    :: sdir_br_tree       ( bounds%begl: , 1: ) ! direct solar radiation (per unit leaf area) incident on below-roof tree per unit incident flux [landunit, numrad]
    real(r8), intent(in)    :: sdif_br_tree       ( bounds%begl: , 1: ) ! diffuse solar radiation (per unit leaf area) incident on below-roof tree per unit incident flux [landunit, numrad]
    real(r8), intent(in)    :: sdir_ar_tree       ( bounds%begl: , 1: ) ! direct solar radiation (per unit leaf area) incident on above-roof tree per unit incident flux [landunit, numrad]
    real(r8), intent(in)    :: sdif_ar_tree      ( bounds%begl: , 1: ) ! diffuse solar radiation (per unit leaf area) incident on above-roof tree per unit incident flux [landunit, numrad]
    real(r8), intent(in)    :: sdir_roof       ( bounds%begl: , 1: ) ! direct solar radiation (per unit roof area) incident on roof per unit incident flux [landunit, numrad]
    real(r8), intent(in)    :: sdif_roof       ( bounds%begl: , 1: ) ! diffuse solar radiation (per unit roof area) incident on roof per unit incident flux [landunit, numrad] 
    ...
    real(r8) :: ar_tree_a_dir(bounds%begl:bounds%endl)         ! absorbed direct solar for above-roof tree (per unit wall area) after "n" reflections per unit incident flux
    real(r8) :: ar_tree_a_dif(bounds%begl:bounds%endl)         ! absorbed diffuse solar for above-roof tree (per unit wall area) after "n" reflections per unit incident flux
    real(r8) :: ar_tree_r_dir(bounds%begl:bounds%endl)         ! reflected direct solar for above-roof tree (per unit wall area) after "n" reflections per unit incident flux
    real(r8) :: ar_tree_r_dif(bounds%begl:bounds%endl)         ! reflected diffuse solar for above-roof tree (per unit wall area) after "n" reflections per unit incident flux
    ...
    real(r8) :: br_tree_a_dir(bounds%begl:bounds%endl)         ! absorbed direct solar for below-roof tree (per unit wall area) after "n" reflections per unit incident flux
    real(r8) :: br_tree_a_dif(bounds%begl:bounds%endl)         ! absorbed diffuse solar for below-roof tree (per unit wall area) after "n" reflections per unit incident flux
    real(r8) :: br_tree_r_dir(bounds%begl:bounds%endl)         ! reflected direct solar for below-roof tree (per unit wall area) after "n" reflections per unit incident flux
    real(r8) :: br_tree_r_dif(bounds%begl:bounds%endl)         ! reflected diffuse solar for below-roof tree (per unit wall area) after "n" reflections per unit incident flux
    ...
    associate(                                                           & 
        kgw1d =>    urbanparams_inst%kgw1d_out  , & ! Input:  [real(r8) (:,:) ]  Shortwave Monte carlo view factor from ground to one wall[landunit, nzcanm]    
        kgv1d =>    urbanparams_inst%kgv1d_out   ,& ! Input:  [real(r8) (:,:) ]  Shortwave Monte carlo view factor from ground to vegetation [landunit, nzcanm]  
        kts1d =>    urbanparams_inst%kts1d_out   ,& ! Input:  [real(r8) (:) ]  Shortwave Monte carlo view factor from ground to sky[landunit, nzcanm]    

        ksg1d =>    urbanparams_inst%ksg1d_out  , & ! Input:  [real(r8) (:) ]    Shortwave Monte carlo view factor from sky to ground[landunit]               
        ksw1d =>    urbanparams_inst%ksw1d_out , & ! Input:  [real(r8) (:,:) ]  Shortwave Monte carlo view factor from sky to wall[landunit, nzcanm]   
        ksr1d =>    urbanparams_inst%ksr1d_out , & ! Input:  [real(r8) (:,:) ]  Shortwave Monte carlo view factor from sky to roof[landunit, nzcanm]                      
        ksv1d =>    urbanparams_inst%ksv1d_out , & ! Input:  [real(r8) (:,:) ]  Shortwave Monte carlo view factor from sky to vegetation[landunit, nzcanm]      
                        
        kwg1d =>    urbanparams_inst%kwg1d_out  , & ! Input:  [real(r8) (:,:) ]      Shortwave Monte carlo view factor of from one wall to ground[landunit]  
        kww1d =>    urbanparams_inst%kww1d_out  , & ! Input:  [real(r8) (:,:,:) ]  Shortwave Monte carlo view factor from on wall to opposing wall [landunit]  
        kwv1d =>    urbanparams_inst%kwv1d_out  , & ! Input:  [real(r8) (:,:,:) ]  Shortwave Monte carlo view factor from on wall to vegetation[landunit]  
        kws1d =>    urbanparams_inst%kws1d_out  , & ! Input:  [real(r8) (:,:) ]    Shortwave Monte carlo view factor from on wall to sky[landunit]  

        kvg1d =>    urbanparams_inst%kvg1d_out  , & ! Input:  [real(r8) (:,:) ]      Shortwave Monte carlo view factor of from vegetation to ground[landunit]  
        kvw1d =>    urbanparams_inst%kvw1d_out  , & ! Input:  [real(r8) (:,:,:) ]  Shortwave Monte carlo view factor from vegetation to one wall [landunit]  
        kvv1d =>    urbanparams_inst%kvv1d_out  , & ! Input:  [real(r8) (:,:,:) ]  Shortwave Monte carlo view factor from vegetation to vegetation[landunit]  
        kvs1d =>    urbanparams_inst%kvs1d_out  , & ! Input:  [real(r8) (:,:) ]    Shortwave Monte carlo view factor from vegetation to sky[landunit]  
        kvr1d =>    urbanparams_inst%kvr1d_out  , & ! Input:  [real(r8) (:,:,:) ]    Shortwave Monte carlo view factor from ovegetation to roof[landunit]  
        
        krs1d =>    urbanparams_inst%krs1d_out , & ! Input:  [real(r8) (:,:) ]  Shortwave Monte carlo view factor from roof to sky [landunit, nzcanm]                 
        krv1d =>    urbanparams_inst%krv1d_out , & ! Input:  [real(r8) (:,:,:) ]  Shortwave Monte carlo view factor  from roof to vegetation [landunit, nzcanm]              

        fgw1d =>    urbanparams_inst%fgw1d_out  , & ! Input:  [real(r8) (:,:) ]  Longwave Monte carlo view factor from ground to  one wall[landunit, nzcanm]    
        fgv1d =>    urbanparams_inst%fgv1d_out  , & ! Input:  [real(r8) (:,:) ]  Longwave Monte carlo view factor from ground to vegetation [landunit, nzcanm]  
        fts1d =>    urbanparams_inst%fts1d_out ,  & ! Input:  [real(r8) (:) ]  Longwave Monte carlo view factor from ground to sky[landunit, nzcanm]    

        fsg1d =>    urbanparams_inst%fsg1d_out   ,& ! Input:  [real(r8) (:) ]    Longwave Monte carlo view factor from sky to ground[landunit]               
        fsw1d =>    urbanparams_inst%fsw1d_out , & ! Input:  [real(r8) (:,:) ]  Longwave Monte carlo view factor from sky to wall[landunit, nzcanm]   
        fsr1d =>    urbanparams_inst%fsr1d_out , & ! Input:  [real(r8) (:,:) ]  Longwave Monte carlo view factor from sky to roof[landunit, nzcanm]                      
        fsv1d =>    urbanparams_inst%fsv1d_out , & ! Input:  [real(r8) (:,:) ]  Longwave Monte carlo view factor from sky to vegetation[landunit, nzcanm]      
                       
        fwg1d =>    urbanparams_inst%fwg1d_out   ,& ! Input:  [real(r8) (:,:) ]      Longwave Monte carlo view factor of from one wall to ground[landunit]  
        fww1d =>    urbanparams_inst%fww1d_out  , & ! Input:  [real(r8) (:,:,:) ]  Longwave Monte carlo view factor from on wall to opposing wall [landunit]  
        fwv1d =>    urbanparams_inst%fwv1d_out  , & ! Input:  [real(r8) (:,:,:) ]  Longwave Monte carlo view factor from on wall to vegetation[landunit]  
        fws1d =>    urbanparams_inst%fws1d_out  , & ! Input:  [real(r8) (:,:) ]    Longwave Monte carlo view factor from on wall to sky[landunit]  

        fvg1d =>    urbanparams_inst%fvg1d_out  , & ! Input:  [real(r8) (:,:) ]      Longwave Monte carlo view factor of from vegetation to ground[landunit]  
        fvw1d =>    urbanparams_inst%fvw1d_out  , & ! Input:  [real(r8) (:,:,:) ]  Longwave Monte carlo view factor from vegetation to one wall [landunit]  
        fvv1d =>    urbanparams_inst%fvv1d_out ,  & ! Input:  [real(r8) (:,:,:) ]  Longwave Monte carlo view factor from vegetation to vegetation[landunit]  
        fvs1d =>    urbanparams_inst%fvs1d_out ,  & ! Input:  [real(r8) (:,:) ]    Longwave Monte carlo view factor from vegetation to sky[landunit]  
        fvr1d =>    urbanparams_inst%fvr1d_out  , & ! Input:  [real(r8) (:,:,:) ]    Longwave Monte carlo view factor from ovegetation to roof[landunit]  

        frs1d =>    urbanparams_inst%frs1d_out , & ! Input:  [real(r8) (:,:) ]  Longwave Monte carlo view factor from roof to sky [landunit, nzcanm]                 
        frv1d =>    urbanparams_inst%frv1d_out , & ! Input:  [real(r8) (:,:,:) ]  Longwave Monte carlo view factor  from roof to vegetation [landunit, nzcanm]              
        ...
        sabs_improad_dir   =>    solarabs_inst%sabs_improad_dir_lun   , & ! Output: [real(r8) (:,:) ]  direct  solar absorbed  by impervious road per unit ground area per unit incident flux
        sabs_perroad_dir   =>    solarabs_inst%sabs_perroad_dir_lun   , & ! Output: [real(r8) (:,:) ]  direct  solar absorbed  by pervious road per unit ground area per unit incident flux
        sabs_br_tree_dir      =>    solarabs_inst%sabs_br_tree_dir_lun      , & ! Output: [real(r8) (:,:) ]  direct  solar absorbed  by below-roof tree per unit leaf area per unit incident flux
        sabs_ar_tree_dir      =>    solarabs_inst%sabs_ar_tree_dir_lun      , & ! Output: [real(r8) (:,:) ]  direct  solar absorbed  by above-roof tree per unit leaf  area per unit incident flux
        sabs_tree_dir      =>    solarabs_inst%sabs_tree_dir_lun      , & ! Output: [real(r8) (:,:) ]  direct  solar absorbed  by tree per unit leaf area per unit incident flux
        sabs_canyon_dir   =>    solarabs_inst%sabs_canyon_dir_lun   , & ! Output: [real(r8) (:,:) ]  direct  solar absorbed  by urban canyon per unit ground area per unit incident flux
        ...
        sabs_improad_dif   =>    solarabs_inst%sabs_improad_dif_lun   , & ! Output: [real(r8) (:,:) ]  diffuse solar absorbed  by impervious road per unit ground area per unit incident flux
        sabs_perroad_dif   =>    solarabs_inst%sabs_perroad_dif_lun    , & ! Output: [real(r8) (:,:) ]  diffuse solar absorbed  by pervious road per unit ground area per unit incident flux
        sabs_br_tree_dif      =>    solarabs_inst%sabs_br_tree_dif_lun      , & ! Output: [real(r8) (:,:) ]  diffuse solar absorbed  by below-roof tree per unit leaf  area per unit incident flux
        sabs_ar_tree_dif      =>    solarabs_inst%sabs_ar_tree_dif_lun      , & ! Output: [real(r8) (:,:) ]  diffuse solar absorbed  by above-roof tree per unit leaf  area per unit incident flux
        sabs_tree_dif      =>    solarabs_inst%sabs_tree_dif_lun      , & ! Output: [real(r8) (:,:) ]  diffuse solar absorbed  by tree per unit leaf area per unit incident flux
        sabs_canyon_dif   =>    solarabs_inst%sabs_canyon_dif_lun   , & ! Output: [real(r8) (:,:) ]  diffuse solar absorbed  by urban canyon per unit ground area per unit incident flux
        ...
        sref_improad_dir     => solarabs_inst%sref_improad_dir_lun     , & ! Output: [real(r8) (:,:) ]  direct  solar reflected by impervious road per unit ground area per unit incident flux
        sref_perroad_dir     => solarabs_inst%sref_perroad_dir_lun     , & ! Output: [real(r8) (:,:) ]  direct  solar reflected by pervious road per unit ground area per unit incident flux
        sref_br_tree_dir     => solarabs_inst%sref_br_tree_dir_lun     , & ! Output: [real(r8) (:,:) ]  direct  solar reflected by below-roof tree per unit leaf area per unit incident flux
        sref_ar_tree_dir     => solarabs_inst%sref_ar_tree_dir_lun     , & ! Output: [real(r8) (:,:) ]  direct  solar reflected by above-roof tree per unit leaf  area per unit incident flux
        sref_tree_dir     => solarabs_inst%sref_tree_dir_lun     , & ! Output: [real(r8) (:,:) ]  direct  solar reflected by tree per unit leaf area per unit incident flux
        sref_canyon_dir     => solarabs_inst%sref_canyon_dir_lun     , & ! Output: [real(r8) (:,:) ]  direct  solar reflected by urban canyon per unit ground area per unit incident flux
        ...
        sref_improad_dif     => solarabs_inst%sref_improad_dif_lun     , & ! Output: [real(r8) (:,:) ]  diffuse solar reflected by impervious road per unit ground area per unit incident flux
        sref_perroad_dif     => solarabs_inst%sref_perroad_dif_lun     , & ! Output: [real(r8) (:,:) ]  diffuse solar reflected by pervious road per unit ground area per unit incident flux
        sref_br_tree_dif     => solarabs_inst%sref_br_tree_dif_lun     , & ! Output: [real(r8) (:,:) ]  diffuse solar reflected by below-roof tree per unit leaf  area per unit incident flux
        sref_ar_tree_dif     => solarabs_inst%sref_ar_tree_dif_lun     , & ! Output: [real(r8) (:,:) ]  diffuse solar reflected by above-roof tree per unit leaf  area per unit incident flux
        sref_tree_dif     => solarabs_inst%sref_tree_dif_lun     , & ! Output: [real(r8) (:,:) ]  diffuse solar reflected by tree per unit leaf area per unit incident flux
        sref_canyon_dif     => solarabs_inst%sref_canyon_dif_lun      & ! Output: [real(r8) (:,:) ]  direct  solar reflected by urban canyon per unit ground area per unit incident flux
         ) 
         ...
         do fl = 1,num_urbanl
            ...
            if (coszen(l) > 0._r8) then
               ...
               if (wtroad_tree(l) > 1e-6_r8) then
                  ...
                  sabs_tree_dif(l,ib) = (sabs_br_tree_dif(l,ib)*A_v1(l) + sabs_ar_tree_dif(l,ib)*A_v2(l))/(A_v1(l)+A_v2(l))

                  ! sabs_tree_dir_per_col  = (sabs_br_tree_dir*A_v1 + sabs_ar_tree_dir*A_v2) / (wtroad_tree*A_g)???
               else
                  sref_tree_dir(l,ib)=0._r8; sref_tree_dif(l,ib)=0._r8
                  sabs_tree_dir(l,ib)=0._r8; sabs_tree_dif(l,ib)=0._r8
               end if
               ! total reflected by canyon - sum of solar reflection to sky from canyon.
               ! project wall fluxes to horizontal surface
                ! In CLM5 code, the sabs_canyon_dif and sref_canyon_dif are in respect to road
               ! Since road tree may attenuation solar radiation on roof, we need to comoute flux inrespect to road+building width (A_s)
               ...
               !write(6,*) 'err =', err
                if (abs(err) > 0.001_r8 ) then
                  ...
                  write(iulog,*)' sref_canyon_dif = ',sref_canyon_dif(l,ib)
                  write(iulog,*) 'clm model is stopping'
                  call endrun(subgrid_index=l, subgrid_level=subgrid_level_landunit, msg=errmsg(sourcefile, __LINE__))
                endif
